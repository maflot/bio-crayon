...
def main():
    ...
    plot_histograms(
        data, cell_colors, ax3, "cell_size", "Cell Size Distribution", "Cell Size (μm)"
    )
    ...
    for cell_type, stat in stats.items():
        print(
            f"{cell_type:<15} {stat['count']:<8} {stat['mean_expression']:<10.2f} "
    ...
    all_expression = np.concatenate(
        [values["expression_level"] for values in data.values()]
    )
    all_activation = np.concatenate(
        [values["activation_score"] for values in data.values()]
    )
    all_size = np.concatenate([values["cell_size"] for values in data.values()])
    all_granularity = np.concatenate(
        [values["granularity"] for values in data.values()]
    )
