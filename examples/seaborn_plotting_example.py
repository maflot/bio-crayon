...
def main():
    ...
    fig.suptitle(
        "Allen Brain Immune Cell Analysis with Seaborn", fontsize=16, fontweight="bold"
    )
    ...
    plt.subplot(2, 3, 4)
    sns.boxplot(data=df, x="cell_type", y="expression_level", palette=color_palette)
    ...
    plt.subplot(2, 3, 5)
    sns.violinplot(
        data=df, x="cell_type", y="activation_score", palette=color_palette, inner="box"
    )
    plt.title("Activation Score Distribution by Cell Type")
    plt.xlabel("Cell Type")
    ...
    sns.pairplot(
        subset_df, hue="cell_type", palette=color_palette, diag_kind="kde", height=2.5
    )
    ...
    sns.heatmap(
        corr_matrix, annot=True, cmap="coolwarm", center=0, square=True, ax=axes[1, 0]
    )
    ...
    print(
        f"Colorblind safe: {bc.metadata.get('accessibility', {}).get('colorblind_safe', 'Unknown')}"
    )
