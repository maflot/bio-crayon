...
def validate_colormap_data(data: Dict[str, Any], require_metadata: bool = False) -> List[str]:
    """
...
def validate_continuous_colormap(colormap: Dict[str, Any]) -> List[str]:
    ...
    for i, pos in enumerate(positions):
        if not isinstance(pos, (int, float)) or pos < 0 or pos > 1:
            errors.append(f"Position at index {i} must be a number between 0 and 1: {pos}")
...
def validate_expression_range(colormap: Dict[str, Any], min_val: float, max_val: float) -> List[str]:
    """
    ...
    if colormap.get("type") != "continuous":
        errors.append("Expression range validation only applies to continuous colormaps")
        return errors
    ...
    if colormap_min > min_val + tolerance:
        errors.append(f"Colormap minimum ({colormap_min}) is higher than expected minimum ({min_val})")
    
    if colormap_max < max_val - tolerance:
        errors.append(f"Colormap maximum ({colormap_max}) is lower than expected maximum ({max_val})")
    
    return errors


def validate_colorblind_safety(colormap: Dict[str, Any], colorblind_type: str = "deuteranopia") -> List[str]:
    """
    ...
    if colormap.get("type") != "categorical":
        errors.append("Colorblind safety validation only applies to categorical colormaps")
        return errors
    ...
    if not is_colorblind_safe(colors, colorblind_type):
        errors.append(f"Colormap colors are not distinguishable for {colorblind_type} colorblind users")
    
    return errors


def validate_bio_specific_requirements(colormap: Dict[str, Any], bio_type: str = "expression") -> List[str]:
    """
    ...
    if bio_type == "expression":
        ...
        if colormap.get("type") == "continuous":
            ...
            if len(colors) >= 2:
                ...
                if distance < 100:  # Threshold for good contrast
                    errors.append("Expression colormap should have high contrast between low and high values")
        ...
        if colormap.get("type") == "continuous":
            ...
            if interpolation not in ["lab", "perceptually_uniform"]:
                errors.append("Structure data should use perceptually uniform color interpolation")
