...
from bio_crayon.utils import (
    hex_to_rgb, rgb_to_hex, rgb_to_hsv, hsv_to_rgb,
    interpolate_colors, load_from_file, load_from_dict,
    detect_source_type, normalize_color, calculate_color_distance,
    rgb_to_lab, lab_to_rgb, interpolate_color_lab,
    is_colorblind_safe, get_colorblind_safe_colors
)
...
class TestColorblindSafety:
    ...
    def test_is_colorblind_safe_unsafe_colors(self):
        ...
        assert is_colorblind_safe(unsafe_colors) == True  # With current threshold, RGB is considered safe
